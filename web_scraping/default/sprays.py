...
def load_roster(code, year_ID):
    ## `load_roster` is a function used to scrape an entire roster from stats.ncaa.org
    ...
    ##   Output:
    ##      roster : the entire roster for the desired team
...
def links_helper(URL, match):
    ## `links_helper` is a helper function for `get_links` that scrapes links matching
    ## a given regex (`match`) from a given URL.
    
    ##   Input:
    ##      URL : the URL that we want to scrape
    ##      match : the regex specifying which links we want to scrape
